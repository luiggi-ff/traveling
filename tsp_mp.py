...
bag_of_tasks=multiprocessing.Queue()
results=multiprocessing.Queue()
min_q=multiprocessing.Queue()
def rand_seq(size):
...
def reversed_sections(tour,max_evaluations):
...
def hillclimb(coords,max_evaluations):
    ...
    while num_evaluations < max_evaluations:
        ...
        move_made=False
        for next in reversed_sections(best,max_evaluations):
...
def hillclimb_and_restart(coords,max_evaluations):
    ...
    camino_min=(evaluated,score,best)
    min_q.put(camino_min)    
    #return (num_evaluations,best_score,best)
...
def main():
    ...
    for i in range(procesos-1):
         w=multiprocessing.Process(target=hillclimb_and_restart, args=(coords,max_iterations))
         w.start()
         jobs.append(w)
